...
void World::updateCamera(float deltaTime)
{
	switch(cameraMode)
	{
	case FIRST_PERSON_CAMERA: updateCamera_FirstPerson(deltaTime); break;
	case OVERHEAD_CAMERA:     updateCamera_Overhead(); break;
	case THIRD_PERSON_CAMERA: updateCamera_ThirdPerson(); break;
	}
}
...
void World::onKeyToggleCamera()
{
	switch(cameraMode)
	{
	case FIRST_PERSON_CAMERA: cameraMode = OVERHEAD_CAMERA;     break;
	case OVERHEAD_CAMERA:     cameraMode = THIRD_PERSON_CAMERA; break;
	case THIRD_PERSON_CAMERA: cameraMode = FIRST_PERSON_CAMERA; break;
	}
...
void World::handleInputKeyPress(const InputKeyPress *input)
{
	switch(input->key)
	{
	case SDLK_F1: onKeyToggleCamera(); break;
	case SDLK_F2: onKeyToggleDebugRendering(); break;
	case SDLK_F3: onKeyTogglePhysics(); break;
	}
}
...
void World::handleInputKeyDown(const InputKeyDown *input)
{
	switch(input->key)
	{
	case SDLK_UP:    onKeyDownUp();      break;
	case SDLK_DOWN:  onKeyDownDown();    break;
	case SDLK_LEFT:  onKeyDownLeft();    break;
	case SDLK_RIGHT: onKeyDownRight();   break;
	case SDLK_i: onKeyDownFwd();         break;
	case SDLK_k: onKeyDownRev();         break;
	case SDLK_j: onKeyDownStrafeLeft();  break;
	case SDLK_l: onKeyDownStrafeRight(); break;
	}
}
