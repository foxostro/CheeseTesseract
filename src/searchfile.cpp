...
vector<FileName> SearchFile(const FileName &_searchDirectory,
	...
	if (hFile != -1L) {
		...
		switch (errno) {
		case EINVAL:
			ERR("Invalid parameter: filespec or fileinfo was NULL. Or, the operating system returned an unexpected error.");
			break;
		case ENOENT:
			ERR("File specification that could not be matched.");
			break;
		case ENOMEM:
