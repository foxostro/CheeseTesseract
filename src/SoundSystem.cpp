...
void SoundSystem::FSOUND_SetSFXMasterVolume(int volume) {
	...
	if ((fmod_err = FSOUND_GetError()) != FMOD_ERR_NONE) {
		TRACE("Failed to set master sfx volume: " + itos(volume) + ": " + FMOD_ErrorString(fmod_err));
	}
#endif
}
...
#ifndef NDEBUG
	if (sound==0) {
		TRACE(string("Failed to load sound file ") + name_or_data + ": " +
...
void SoundSystem::FSOUND_StopSound(int channel) {
	...
	if (!r) {
		TRACE(string("Failed to stop all sounds: ") +
		     FMOD_ErrorString(FSOUND_GetError()));
	}
#else
...
#ifndef NDEBUG
	if (!r) {
		TRACE(string("Failed to initialize FMOD: ") +
		     FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
...
void SoundSystem::FSOUND_SetMute(int channel, signed char mute) {
	...
	if (!r) {
		if (mute) {
			TRACE("Failed to mute channel " + itos(channel) + ": " + FMOD_ErrorString(FSOUND_GetError()));
		} else {
			TRACE("Failed to unmute channel " + itos(channel) + ": " + FMOD_ErrorString(FSOUND_GetError()));
		}
	}
#else
...
#ifndef NDEBUG
	if (!stream) {
		TRACE(string("Failed to open stream ") + name_or_data + ": "
		     + FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
...
#ifndef NDEBUG
	if (r == -1) {
		TRACE(string("Failed to play stream: ")
		     + FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
...
#ifndef NDEBUG
	if (!r) {
		TRACE(string("Failed to set channel volume: ")
		     + FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
...
#ifndef NDEBUG
	if (!r) {
		TRACE(string("Failed to stop stream: ")
		     + FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
...
#ifndef NDEBUG
	if (!r) {
		TRACE(string("Failed to close stream: ")
		     + FMOD_ErrorString(FSOUND_GetError()));
	}
#endif
